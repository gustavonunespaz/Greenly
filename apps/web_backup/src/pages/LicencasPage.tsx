...
export default function LicencasPage() {
  ...
  const filtered = filter === "TODAS" ? licencas : licencas.filter((l) => l.status === filter);
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left text-[11px] text-muted-foreground/70 font-medium uppercase tracking-wider px-5 py-3">Cliente</th>
                  ...
                  <th className="text-left text-[11px] text-muted-foreground/70 font-medium uppercase tracking-wider px-5 py-3">Órgão</th>
                ...
                {filtered.map((lic, i) => (
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <p className="text-sm font-medium text-foreground">{lic.cliente}</p>
                      <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-[200px]">{lic.empreendimento}</p>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-sm text-muted-foreground">{lic.orgao}</td>
                    <td className="px-5 py-3.5 text-sm tabular-nums text-muted-foreground">{lic.dataValidade}</td>
                    <td className="px-5 py-3.5">
                      <span className={`text-sm tabular-nums font-medium ${
                        ...
                        {lic.diasRestantes < 0 ? `${Math.abs(lic.diasRestantes)}d atrás` :
                         lic.diasRestantes === 0 ? "—" : `${lic.diasRestantes}d`}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
